...
export default function Register() {
  ...
  async function registerWithGoogle() {
    ...
    try {
      ...
      // Treat user-cancel as a plain cancel (don’t loop)
      const msg = niceFirebaseError(e.code, e.message);
      setErr(msg);
    } finally {
      setLoading(false);
    }
  ...
  async function resendOtp() {
    ...
    if (!auth.currentUser?.uid)
      return setErr("You must be signed in to resend the OTP.");
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/api/auth/register`, {
        method: "POST",
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] grid place-items-center bg-gradient-to-b from-white via-indigo-50 to-white">
      <div
        className="w-full max-w-md bg-white border rounded-2xl shadow-sm p-6 relative"
        aria-busy={loading}
        aria-live="polite"
      >
        {loading && (
        ...
        {!otpSent ? (
          ...
          >
            <fieldset disabled={loading} className="space-y-4">
              ...
              <button
                type="submit"
                className="w-full inline-flex items-center justify-center gap-2 rounded-lg bg-black px-4 py-2 text-white hover:opacity-95 disabled:opacity-60"
                disabled={loading}
              >
              ...
              <button
                type="button"
                className="w-full inline-flex items-center justify-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2 hover:bg-gray-50 disabled:opacity-60"
