...
const parseSkills = (v) => v.split(/,\s*/g).map(s=>s.trim()).filter(Boolean);
...
export default function WorkerProfile() {
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white border rounded-xl p-6 space-y-4">
      <h1 className="text-2xl font-semibold">Worker Profile</h1>
      ...
      <input className="w-full border rounded px-3 py-2" placeholder="Skills (comma separated)"
             value={(form.skills||[]).join(", ")}
             onChange={(e)=>setForm(p=>({...p, skills: parseSkills(e.target.value)}))}/>
